    ...
    <>
      ...
      <main className="mx-auto w-full max-w-5xl px-5 pb-24 sm:px-8">
        <section className="border-b border-rule py-8 sm:py-14">
          ...
          <h1 className="headline mt-2 text-2xl sm:text-5xl">Choose your recipe</h1>
          ...
          <ul className="grid gap-px bg-rule sm:grid-cols-2">
            {RECIPES.map((recipe) => (
              <li key={recipe.id} className="bg-paper">
                ...
                >
                  <p className="meta text-sm uppercase tracking-[0.16em]">
                    {recipe.author} · {recipe.level}
                  </p>

                  <h2 className="headline mt-2 text-xl transition-colors group-hover:text-accent sm:text-2xl">
                    {recipe.name}
                  </h2>

                  <p className="summary mt-3 grow text-base">{recipe.blurb}</p>
                  ...
                  <dl className="mt-4 flex flex-wrap items-baseline gap-x-6 gap-y-2 border-t border-rule pt-4">
                    <div className="flex items-baseline gap-2">
                      ...
                      <dd className="tnum headline text-lg">
                        {formatClock(recipe.totalSeconds)}
                      </dd>
                    </div>
                    <div className="flex items-baseline gap-1.5">
                      <dd className="tnum headline text-lg">{recipe.dose}g</dd>
                      <dt className="meta text-sm">coffee</dt>
                    </div>
                    <div className="flex items-baseline gap-1.5">
                      <dd className="tnum headline text-lg">{recipe.water}g</dd>
                      <dt className="meta text-sm">water</dt>
                    </div>
                  </dl>

                  <p className="kicker mt-4 text-xs transition-transform group-hover:translate-x-1">
                    Start this brew →
                  </p>
                </Link>
              </li>
        ...
        <section className="border-t border-rule bg-paper-soft p-5 sm:p-8">
          ...
          <p className="summary mt-2 max-w-2xl text-base">
